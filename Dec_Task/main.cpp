...
int main () {
	try {
	int quantity = 0;
	std::cout << "Введите количество цифр самого большого вашего числа:" << std::endl;
	std::cin >> quantity;
	if (std::cin.fail()) { throw std::invalid_argument ("Зачем вам программа, которая не считает ничего?"); }

	std::cout << "Вводите числа по уровням дерева: " << std::endl;
	std::cout << "Уровень " << 1 << std::endl;
	int num = 0;
	std::cin >> num;
	if (std::cin.fail()) throw std::invalid_argument ("Введено некорректное значение.");
